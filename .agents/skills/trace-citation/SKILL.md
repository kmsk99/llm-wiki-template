...
3. **Graphify 경로 확인 (보조):** `graphify path "<wiki 노드명>" "<raw 노드명>"`으로 그래프상의 경로를 함께 뽑는다. 파일 frontmatter의 source와 그래프 경로가 일치하지 않으면 🟡 또는 ❌ 후보로 우선 검토한다.
4. 해당 raw 소스를 읽고, wiki 문서의 핵심 사실이 실제로 raw에 있는지 **파일 본문으로 최종 검증**한다. 그래프는 힌트일 뿐, 확정 증거는 파일이다.
5. 각 사실에 대해 추적 결과를 표시한다:
   - ✅ **Traced**: raw 소스에서 직접 확인됨
   - 🟡 **Inferred**: raw에 암시되어 있으나 명시적이지 않음 (또는 그래프 경로가 AMBIGUOUS)
   - ❌ **Untraced**: raw 소스에서 근거를 찾을 수 없음
   - 🔄 **Multi-source**: 여러 raw 소스에서 확인됨

### 사실 단위 추적
1. 키워드로 wiki 문서를 검색한다.
   - 1차: `graphify explain "<사실/키워드>"`로 해당 개념을 품은 노드와 엣지를 확인한다.
   - 2차: Grep으로 정확한 문구가 들어간 파일을 확인한다.
