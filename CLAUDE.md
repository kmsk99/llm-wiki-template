...
설치 항목 (5단계):
1. 시스템 의존성 — poppler (pdftotext), ffmpeg (오디오 변환), Node.js (QMD), curl
2. Python 3.12 + Docling + opendataloader-pdf (Java 11+ JRE) + 보조 파서 — 3.10 미만이면 자동 설치
3. QMD 검색 엔진 — GGUF 모델 3종 (embedding + reranking + generation) + collection 생성 + 인덱싱
4. 디렉토리 구조 — raw/, wiki/, output/
5. 설정 파일 — Claude Code MCP, manifest, index

플래그: `--skip-python`, `--skip-qmd`, `--skip-models`, `-h`
