...
- **비텍스트 파일은 하이브리드 파이프라인으로 변환한다.**
  - PDF → `scripts/parse-pdf.py` (opendataloader-pdf Java JAR + CLIPROXY 이미지 캡션)
  ...
  - `scripts/repair_parsed_artifacts.py`로 stale parsed/중복 파일명을 복구할 수 있다.
  - repo 루트 `.env`의 `CLIPROXY_*` / `DOCLING_*` 값을 자동으로 읽는다.
...
| `scripts/parse-raw.sh` | 비텍스트 파일 → Markdown 파싱 라우터 |
| `scripts/parse-pdf.py` | PDF 전용 파서 (opendataloader-pdf + CLIPROXY 캡션) |
| `scripts/parse_docling.py` | DOCX/PPTX/XLSX/이미지 Docling 엔진 |
| `scripts/parse-html.py` | HTML 본문 전용 파서 |
| `scripts/parse-hwp.py` | HWP/HWPX 전용 파서 |
| `scripts/parse-image.py` | GPT Vision 우선 이미지 파서 |
| `scripts/parse-text.py` | TXT/DOC 전용 파서 |
| `scripts/repair_parsed_artifacts.py` | parsed 복구 및 파일명 정리 |
...
| `.claude/commands/` | 12개 위키 운영 스킬 (Claude Code slash commands) |
| `.claude/settings.json` | Claude Code hook 설정 (graphify 힌트) |
| `.agents/skills/` | 12개 위키 운영 스킬 (Codex CLI skills) |
...
설치 항목 (5단계):
1. 시스템 의존성 — poppler (pdftotext), ffmpeg (오디오 변환), curl
2. Python 3.12 + Docling + opendataloader-pdf (Java 11+ JRE) + 보조 파서 — 3.10 미만이면 자동 설치
3. Graphify 지식 그래프 도구 — CLI + graph build/update workflow
4. 디렉토리 구조 — raw/, wiki/, output/
...
### Tier 1 — 기본 루프
- **Catalog** (`/project:catalog`): raw 소스를 `raw/.manifest.md`에 등록하고 하이브리드 파서(`parse-raw.sh`)를 실행한다. PDF는 opendataloader-pdf + CLIPROXY 이미지 캡션, HTML/HWP/HWPX/이미지/TXT/DOC는 전용 파서, 그 외는 Docling을 사용한다.
- **Ingest** (`/project:ingest`): raw → wiki 승격. 소스 읽기 → 분석 → 문서 생성/갱신 → 색인 갱신.
- **Batch Ingest** (`/project:batch-ingest`): 대량 파일 일괄 승격. promote(빠른 승격) / merge(주제 통합) / selective(선별) 모드 지원.
...
- 파싱 결과: `raw/docs/filename.parsed.md` (원본 옆에 생성)
...
그래프 기반 탐색은 Graphify를 기본으로 사용한다. 정확한 문자열/경로 확인은 Grep/Glob으로 보완한다.

### 검색 우선순위 (Hard Rule)

**주제, 구조, 관계, 출처 흐름을 탐색할 때는 Graphify를 먼저 사용한다. 정확한 문자열·정규식·파일 경로 확인만 Grep/Glob으로 처리한다.**

| 상황 | 사용 도구 | 이유 |
|------|-----------|------|
| wiki/raw/output 내용의 주제·관계·구조 탐색 | `graphify-out/GRAPH_REPORT.md`, `/graphify query|path|explain` | 개념 관계와 커뮤니티 구조를 함께 보여줌 |
| 그래프가 없거나 오래됨 | `/graphify <path>` / `/graphify . --update` | 현재 코퍼스를 기준으로 그래프 재구축 |
| 코드·설정·스크립트 파일 검색 (`.sh`, `.json`, `.yml` 등) | Grep / Glob | 정확한 파일/패턴 탐색에 유리 |
| 정확한 문자열·정규식 매칭이 필요할 때 | Grep | 패턴 매칭은 Grep이 정확 |
| 파일 존재 여부·경로 패턴 확인 | Glob | 파일시스템 탐색 |

### Graphify 사용법
- **Claude Code 내부**: 먼저 `graphify-out/GRAPH_REPORT.md`를 읽고 `/graphify query`, `/graphify path`, `/graphify explain`을 사용한다.
- **터미널**: `graphify query "질문"`, `graphify path "A" "B"`, `graphify explain "개념"`.
- wiki/raw/output이 바뀌면 `graphify . --update` 또는 범위를 좁힌 `/graphify wiki`, `/graphify output`으로 갱신한다.
- `raw/`는 불변 원본이므로, raw 내부의 과거 검색 도구 언급은 역사적 기록으로 취급하고 현재 운영 규칙으로 따르지 않는다.
