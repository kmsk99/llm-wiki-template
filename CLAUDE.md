...
| `scripts/parse-raw.sh` | 비텍스트 파일 → Markdown 파싱 (pdftotext + MarkItDown) |
...
2. Python 3.12 + markitdown — 3.10 미만이면 자동 설치
...
### Tier 1 — 기본 루프
- **Catalog** (`/project:catalog`): raw 소스를 `raw/.manifest.md`에 등록, 비텍스트 파일 파싱 (PDF → pdftotext, 그 외 → MarkItDown).
