...
- **비텍스트 파일(PDF, XLSX, DOCX, PPTX, 이미지 등)은 Marker로 파싱한다.**
  - `scripts/parse-raw.sh`를 실행하면 `*.parsed.md` 파일이 원본 옆에 생성된다.
  - Ingest 시 원본이 아닌 `.parsed.md`를 읽어 wiki로 승격한다.
  - 기본으로 CLIProxyAPI + LLM을 사용하여 테이블/폼 정확도를 높인다 (비전 기반 보정).
...
| `scripts/setup.sh` | 초기 환경 셋업 (Python, marker-pdf, 디렉토리 구조 확인) |
| `scripts/parse-raw.sh` | Marker 기반 비텍스트 파일 → Markdown 파싱 스크립트 |
...
| `.claude/commands/` | 11개 위키 운영 스킬 (slash commands) |
...
11개 운영 스킬이 `.claude/commands/`에 정의되어 있다. `/project:<스킬명>`으로 호출한다.
상세 명세는 [SCHEMA.md](SCHEMA.md) 참조.

### Tier 1 — 기본 루프
- **Catalog** (`/project:catalog`): raw 소스를 `raw/.manifest.md`에 등록, 비텍스트 파일 Marker 파싱.
...
에이전트가 작업할 때는 `.claude/commands/`의 스킬을 사용한다.
수동으로 raw → wiki 승격을 할 때의 절차:

1. `/project:catalog <파일>` — raw 소스 등록 + 비텍스트 파싱.
2. `/project:ingest <파일>` — wiki로 승격 (중복/모순 확인 → 문서 생성/갱신 → 색인 갱신 → 보고).
3. 정보 변경 시 `/project:supersede <소스>` — 기존 문서 갱신 + Change log 기록 + 파급 효과 추적.

각 스킬의 상세 절차는 해당 `.claude/commands/*.md` 파일과 [SCHEMA.md](SCHEMA.md)에 명세되어 있다.
...
QMD를 사용하여 위키를 검색할 수 있다. 프로젝트 초기화 후 `qmd init`으로 collection을 생성한다.
...
| 상황 | 사용 도구 | 이유 |
|------|-----------|------|
| wiki/raw/output 내용 검색 (의미, 주제, 질문) | `mcp__plugin_qmd_qmd__query` | 시맨틱+키워드 하이브리드 검색, 랭킹 제공 |
| 특정 문서 전문 조회 | `mcp__plugin_qmd_qmd__get` / `mcp__plugin_qmd_qmd__multi_get` | 경로/docid로 빠른 조회 |
...
### QMD 사용법
- **Claude Code 내부**: `mcp__plugin_qmd_qmd__query` 도구로 검색 (MCP 서버 `.claude/settings.json`에 설정됨).
